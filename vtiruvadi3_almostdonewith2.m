...
function [V_o, E_o] = obst_energy(obstData)
    angles = (2 * pi * (0:15) ./ 16)';
    
    obst_vect = [obstData .* cos(angles),obstData .* sin(angles)];
    
    vect_weights = (100./((10*obstData).^2)).^4;
    
    inv_E = repmat(vect_weights,1,2);
    V = inv_E .* obst_vect;
    V_o = V;
    for ii = 1:16
        V_o(ii,isnan(V_o(ii,:))) = 0;
    end
    V_o = -sum(V_o,1);
    E_o = 1./(norm(V_o));
end
...
function [u,saveData]=controller2(uid,nbrData,wpData,obstacleData,missionData,saveData,delta,agentRadius,firstCall)
    ...
    if Ms(1) > 2
        AA_E = dist_energy(nbrData,7);
    else
        AA_E = dist_energy(nbrData,delta/15);
    end
    ...
    if uid ~= 1
        %Need to be ok with breaking edges
        
        for ii = 1:Ms(1)
           if lined
            u = u + AA_E(ii) * nbrData(ii,1:2)';%% + O_E * O_V';
           end
        end
        u = u + O_gain * O_V;
    else
        %fixed_WP_V = proj_W_O(wpData,O_V)
        u = (500/norm(wpData)) * wpData + (500/norm(O_V)) * O_V;
    end
